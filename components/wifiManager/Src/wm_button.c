...
void IRAM_ATTR gpio_isr_handler(void* arg) {
		...
		xEventGroupSetBitsFromISR(wm_task_event_group, BUTTON_PRESSED_BIT, &xHigherPriorityTaskWoken);
...
esp_err_t button_init() {
	...
	#ifdef GPIO_INTR_POSEDGE
	io_conf.intr_type = GPIO_INTR_POSEDGE;
	#endif
	#ifdef GPIO_INTR_NEGEDGE
...
void button_task(void *pvParameters) {
	...
	while(1) {
		...
		if((xEventGroupGetBits(wm_main_event_group) & WM_EVENTG_MAIN_HTTP_OPEN) != 0) {
